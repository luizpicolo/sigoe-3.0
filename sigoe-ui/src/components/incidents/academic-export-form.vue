...
const submit = async () => {
  ...
  formData.append('data[Atendimento][atendimento_tipo_id]', atendimentoTipoId.value)
  formData.append('data[Atendimento][descricao]', descricao.value)
  formData.append('data[Atendimento][data_atendimento]', dataAtendimento.value)
  formData.append('data[Atendimento][processo_suap]', processoSuap.value)

  try {
    const response = await fetch(props.action, { method: 'POST', body: formData, credentials: 'include' })
    if (!response.ok) throw new Error('Falha ao enviar a ocorrência')
    message.value = 'Ocorrência submetida com sucesso ao Sistema Acadêmico.'
  } catch (submissionError) {
    error.value = true
    message.value = 'Não foi possível submeter a ocorrência. Verifique se você está logado no Sistema Acadêmico e se o acesso externo permite o envio.'
  } finally {
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
    <form :action="action" method="post" class="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl" @submit.prevent="submit">
      ...
      <p class="mb-4 rounded bg-yellow-100 p-3 text-sm text-yellow-800">Esta é uma funcionalidade experimental. Certifique-se de estar logado no Sistema Acadêmico antes de enviar os dados.</p>
      ...
      <p v-if="message" class="mb-4 rounded p-3 text-sm" :class="error ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'">{{ message }}</p>
      <div class="flex justify-end gap-2">
        <button type="button" class="rounded border px-4 py-2" @click="emit('close')">Cancelar</button>
        <button type="submit" :disabled="submitting" class="rounded bg-green-600 px-4 py-2 text-white hover:bg-green-700 disabled:opacity-50">{{ submitting ? 'Enviando...' : 'Salvar' }}</button>
      </div>
    </form>
  </div>
</template>