...
const breadcrumbItems = [
  ...
  { label: 'Turmas', href: '/administrador/turmas/listar' }
]
...
const filteredGroups = computed(() => {
  ...
  return groups.value.filter(group =>
    [group.id, group.name, group.identifier, group.polo?.name]
...
const loadGroups = async () => {
  ...
  try {
    ...
    groups.value = response?.school_groups || []
  } catch (e) {
      ...
      <main class="flex-1 min-w-0 p-6">
        ...
        <div class="bg-white rounded-md shadow p-4 mb-6">
          <div class="flex flex-wrap gap-2">
            <Button v-if="can('classes', 'create')" to="/administrador/turmas/novo" customClass="bg-green-600 hover:bg-green-700 focus:ring-green-500">
        ...
        <div class="bg-white rounded-md shadow overflow-hidden w-full">
          <div v-if="loading" class="p-6 text-center">Carregando turmas...</div>
          <table v-else class="w-full table-fixed divide-y divide-gray-200">
            <thead class="bg-gray-100"><tr>
              ...
              <th class="w-[20%] px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Identificador</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="group in filteredGroups" :key="group.id">
                <td class="px-3 py-4 text-sm font-medium text-gray-900 break-words">{{ group.id }}</td>
                <td class="px-3 py-4 text-sm text-gray-500 break-words">{{ group.name }}</td>
                <td class="px-3 py-4 text-sm text-gray-500 break-words">{{ group.identifier }}</td>
                <td class="px-3 py-4 text-sm text-gray-500 break-words">{{ group.polo?.name || '-' }}</td>
                <td class="px-3 py-4 text-sm text-gray-500"><div class="flex flex-col gap-2"><Button v-if="can('classes', 'update')" :to="`/administrador/turmas/editar/${group.id}`" customClass="w-full bg-blue-600 hover:bg-blue-700">Editar</Button><Button v-if="can('classes', 'destroy')" @click="destroy(group.id)" customClass="w-full bg-red-600 hover:bg-red-700">Excluir</Button></div></td>
              </tr>
              <tr v-if="!filteredGroups.length"><td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">Nenhuma turma encontrada.</td></tr>
            </tbody>
          </table>
        </div>
      </main>
