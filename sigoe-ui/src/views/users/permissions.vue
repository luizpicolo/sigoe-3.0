...
const availableEntities = computed(() => {
  return entities.value.filter(entity => {
    return !permissions.value.some(permission => permission.entity === entity.id)
  })
})

const normalizePermissions = rawPermissions => {
  return Object.entries(rawPermissions || {}).map(([entity, values]) => ({
    entity,
    ...values
  }))
}
...
const load = async () => {
  ...
  try {
    ...
    error(e.response?.data?.error || 'Não foi possível carregar as permissões deste usuário.')
  } finally {
    loading.value = false
  }
}
...
const addEntity = entityId => {
  if (!entityId || permissions.value.some(permission => permission.entity === entityId)) {
    return
  }
...
const save = async () => {
  ...
  try {
    ...
    error(e.response?.data?.error || e.response?.data?.errors?.join(', ') || 'Não foi possível salvar as permissões.')
  } finally {
    saving.value = false
  }
}
...
const isOccurrence = permission => {
  return permission.entity === 'occurrences'
}
      ...
      <main class="flex-1 p-6">
        ...
        <div class="flex flex-wrap justify-between items-center gap-3 mb-4">
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">
              Defina quais telas <strong>{{ selectedUserName }}</strong> pode acessar e quais operações pode realizar.
            </p>
          </div>
        ...
        <Card :title="`Permissões de ${selectedUserName}`">
          <div v-if="loading" class="py-10 text-center text-gray-500">
            Carregando permissões de {{ selectedUserName }}...
          </div>
          ...
          <template v-else>
              ...
              <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  ...
                  <p class="text-sm text-gray-500">
                    Adicione uma tela para configurar exatamente o que o usuário poderá fazer nela.
                  </p>
                </div>
              ...
              <div class="px-4 py-3 bg-gray-100 flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p class="text-xs text-gray-500">
                    Permissões disponíveis para esta tela
                  </p>
                </div>
            ...
            <div class="mt-6 p-4 rounded-md border border-gray-200 text-sm text-gray-600">
              <p>
                <strong>Visualizar / Listar</strong> corresponde à permissão <code>can_read</code> usada pelo Rails para leitura da entidade.
              </p>

              <p class="mt-1">
                Assim, por exemplo, você pode adicionar <strong>Estudantes</strong> e marcar somente duas operações, como <strong>Visualizar / Listar</strong> e <strong>Atualizar</strong>. As demais permanecem desmarcadas.
              </p>
            </div>
