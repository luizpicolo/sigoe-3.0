      ...
      <main class="flex-1 p-6">
        ...
        <div class="bg-white rounded-md shadow p-4 mb-6">
          <div class="flex flex-wrap gap-2">
            <Button :disabled="!can('users', 'create')"
              customClass="bg-green-600 hover:bg-green-700 focus:ring-green-500" to="/administrador/usuarios/novo"><i
                class="fa-solid fa-user"></i>Novo Usuário</Button>
            <div class="flex items-center gap-2 ml-auto"><span class="text-sm">Ordenar por</span>
              <div class="relative w-[180px]"><select @change.prevent="handleOrderBy"
                  class="block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm">
                  <option value="id">ID</option>
                  <option value="name">Nome</option>
                </select></div>
            </div>
            <div class="flex items-center gap-2"><span class="text-sm">Total</span>
              <div class="relative w-[80px]"><select
                  class="block w-full px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm">
            ...
            <div class="flex items-center gap-2"><Input v-model="search" type="text" placeholder="Buscar..."
                class="w-[200px]" @keyup.enter="handleSearch" /><button
                class="justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700"
                @click="handleSearch"><i class="fa-solid fa-magnifying-glass"></i>Busca</button></div>
          </div>
        </div>
        <div class="bg-white rounded-md shadow overflow-hidden">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody>
              <tr v-for="user in users" :key="user.id" class="bg-white divide-y divide-gray-200">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  <Button customClass="w-full bg-green-600 hover:bg-green-700 focus:ring-green-500"
                    :to="`/administrador/usuarios/visualizar/${user.id}`">
          ...
          <div class="flex justify-end items-end p-4">
            <VPagination v-model="page" :pages="Math.max(1, Math.ceil(usersTotal / 12))" :range-size="2"
              active-color="#00a63e" @update:modelValue="handlerUpdatePagination" />
          </div>
        </div>
      </main>
    </div>
