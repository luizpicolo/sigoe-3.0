...
const handleSubmit = async () => {
  ...
  else successMessage.value = 'Ocorrência atualizada com sucesso.'
  saving.value = false
}
...
<template>
  <div class="min-h-screen flex flex-col">
    <Header />
    <div class="flex flex-col md:flex-row flex-1"><Sidebar :activePage="'ocorrencias'" /><main class="flex-1 p-6"><Breadcrumb :items="breadcrumbItems" /><h1 class="text-2xl font-bold mb-6">Editar Ocorrência #{{ incidentId }}</h1>
      <div v-if="loading" class="p-4">Carregando ocorrência...</div>
      <form v-else @submit.prevent="handleSubmit" class="space-y-6">
        <Card title="Estudante"><div class="grid grid-cols-6 gap-6"><div class="col-span-4"><p class="text-sm font-medium text-gray-700">Estudante</p><p class="mt-2">{{ incident?.student?.name || 'Não informado' }}</p><p class="text-sm text-gray-500">R.A.: {{ incident?.student?.ra || 'Não informado' }}</p><p class="text-sm text-gray-500">Turma: {{ incident?.course?.name || 'Não informado' }}</p><p class="text-sm text-gray-500">Campus: {{ campusName }}</p></div><div class="col-span-2"><Select id="studentType" label="Estudante é?" v-model="selectedStudentType" :options="[{ value: 'non_resident', label: 'Não residente' }, { value: 'resident', label: 'Residente' }]" /></div></div></Card>
        ...
        <Card title="Descrição da solução"><textarea v-model="solutionDescription" rows="5" class="w-full border rounded-md" placeholder="Solução"></textarea></Card>
        <p v-if="errorMessage" class="text-red-600">{{ errorMessage }}</p><p v-if="successMessage" class="text-green-600">{{ successMessage }}</p><div class="flex gap-2"><Button type="submit" :disabled="saving">{{ saving ? 'Salvando...' : 'Atualizar' }}</Button><Button :to="`/ocorrencias/ocorrencias/visualizar/${incidentId}`" customClass="bg-white border-gray-200 !text-gray-900">Cancelar</Button></div>
      </form>
    </main></div>
  </div>
</template>
