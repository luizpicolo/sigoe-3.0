...
const handleSubmit = async () => {
  ...
  if (!selectedStudentIds.value.length) {
    return (errorMessage.value = 'Selecione pelo menos um estudante.')
  }
            ...
            <div class="grid grid-cols-6 gap-6">
              <div class="col-span-4">
                ...
                <div class="rounded-md mt-2 max-h-56 overflow-y-auto border-2 border-gray-300 bg-white">
                  <div v-if="loadingStudents" class="p-3 text-sm">
                    Buscando...
                  </div>

                  <label v-for="student in visibleStudents" :key="student.id" class="flex gap-2 p-2">
                    <input type="checkbox" :checked="isSelected(student.id)" @change="toggleStudent(student)">
                    <span>
