...
const selectedStudentType = ref(editing ? '' : 'non_resident')
...
const occurrenceDate = ref(new Date().toISOString().split('T')[0])
const occurrenceTime = ref(new Date().toTimeString().slice(0, 5))
...
const breadcrumbItems = computed(() => [
  ...
  { label: 'Ocorrências', href: '/ocorrencias/ocorrencias/listar' },
  {
    ...
    href: editing ? `/ocorrencias/ocorrencias/editar/${incidentId}` : '/ocorrencias/ocorrencias/nova'
  }
])
...
const loadIncident = async () => {
  ...
  const response = await findIncident(incidentId)
...
const loadData = async () => {
  ...
  try {
    ...
    error(e.response?.data?.errors?.join(', ') || e.response?.data?.error || `Não foi possível carregar ${editing ? 'a ocorrência.' : 'as opções da ocorrência.'}`)
  } finally {
    loading.value = false
  }
}
...
const handleSubmit = async () => {
  if (!editing && !selectedStudentIds.value.length) {
    ...
    <div class="flex flex-col md:flex-row flex-1">
      ...
      <main class="flex-1 p-6">
        ...
        <h1 class="text-2xl font-bold mb-6">
          {{ editing ? `Editar Ocorrência #${incidentId}` : 'Cadastrar Nova Ocorrência' }}
        </h1>

        <div v-if="loading">
          Carregando {{ editing ? 'ocorrência' : 'opções' }}...
        </div>
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <Card title="Ocorrência">
            ...
            <textarea v-model="occurrenceDescription" rows="5" class="w-full border rounded-md mt-4 p-3" placeholder="Ocorrência"></textarea>
