...
const restrictedReadOnly = computed(() => (
  permissionState.permissions?.occurrences?.can_read_restricted === true &&
  !permissionState.admin &&
  permissionState.user?.super_admin !== true
))

const canAccessIncident = incident => (
  (!restrictedReadOnly.value &&
    (incident.visibility !== 'private' ||
      permissionState.admin ||
      permissionState.user?.super_admin === true ||
      incident.user?.id === permissionState.user?.id)) ||
  (restrictedReadOnly.value && incident.user?.id === permissionState.user?.id)
)
        ...
        <div class="bg-white rounded-md shadow p-4 mb-6">
          <div class="flex flex-wrap gap-2">
            ...
            <div class="flex items-center gap-2 ml-auto">
              ...
              <select v-model="order" @change="changeFilters" class="block w-[180px] px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm">
            ...
            <div class="flex items-center gap-2">
              ...
              <select v-model="amount" @change="changeFilters" class="block w-[80px] px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm">
          ...
          <table v-else class="w-full table-fixed divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="incident in incidents" :key="incident.id">
                ...
                <td v-if="showCampus" class="px-3 py-4 text-sm">
                  {{ incident.course?.polo?.name || '-' }}
                </td>
