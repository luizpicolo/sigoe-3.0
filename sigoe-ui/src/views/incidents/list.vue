...
const restrictedReadOnly = computed(() => permissionState.permissions?.occurrences?.can_read_restricted === true && !permissionState.admin && permissionState.user?.super_admin !== true)

const canAccessIncident = incident => !restrictedReadOnly.value && (incident.visibility !== 'private' || permissionState.admin || permissionState.user?.super_admin === true || incident.user?.id === permissionState.user?.id) || restrictedReadOnly.value && incident.user?.id === permissionState.user?.id
    ...
    <div class="flex flex-col md:flex-row flex-1 min-w-0">
      ...
      <main class="flex-1 min-w-0 p-6">
        ...
        <div class="bg-white rounded-md shadow p-4 mb-6">
          <div class="flex flex-wrap gap-2"><Button :disabled="!can('occurrences', 'create')" customClass="bg-green-600 hover:bg-green-700 focus:ring-green-500" to="/ocorrencias/ocorrencias/novo"><i class="fa-solid fa-user"></i>Nova Ocorrência</Button>
            <div class="flex items-center gap-2 ml-auto"><span class="text-sm">Ordenar por</span><select v-model="order" @change="changeFilters" class="block w-[180px] px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm"><option value="id">ID</option><option value="date_incident">Data</option></select></div>
            <div class="flex items-center gap-2"><span class="text-sm">Total</span><select v-model="amount" @change="changeFilters" class="block w-[80px] px-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm"><option :value="10">10</option><option :value="25">25</option><option :value="50">50</option><option :value="100">100</option></select></div>
            <div class="flex items-center gap-2"><Input v-model="search" type="text" placeholder="Buscar..." class="w-[200px]" /><Button @click="searchIncidents" customClass="bg-green-600 hover:bg-green-700 focus:ring-green-500"><i class="fa-solid fa-magnifying-glass"></i>Busca</Button></div>
          </div>
        </div>
