...
<div class="rounded-xl bg-white p-4 shadow-sm"><h2 class="font-semibold">Acesso</h2><div class="mt-4 space-y-3"><input v-model="form.username" required placeholder="Nome de usuário" class="w-full rounded-lg border px-3 py-3"><input v-model="form.password" type="password" :required="!editing" placeholder="Senha" class="w-full rounded-lg border px-3 py-3"><input v-model="form.password_confirmation" type="password" :required="!editing" placeholder="Confirmar senha" class="w-full rounded-lg border px-3 py-3"><label class="flex items-center gap-2 text-sm"><input type="checkbox" v-model="form.admin"> Administrador</label><label class="flex items-center gap-2 text-sm"><input type="checkbox" v-model="form.status"> Usuário ativo</label></div></div>
<div class="rounded-xl bg-white p-4 shadow-sm"><label class="text-sm font-medium">Campus<select v-model="form.polo_id" class="mt-1 w-full rounded-lg border px-3 py-3"><option value="">Selecione</option><option v-for="p in campi" :key="p.id" :value="p.id">{{p.name}}</option></select></label></div>
<div v-if="errorMessage" class="rounded-xl bg-red-50 p-4 text-sm text-red-700">{{errorMessage}}</div><div class="grid grid-cols-2 gap-2"><RouterLink :to="editing?`/mobile/users/${route.params.id}`:'/mobile/users'" class="rounded-lg bg-gray-100 px-4 py-3 text-center">Cancelar</RouterLink><button :disabled="loading" class="rounded-lg bg-green-600 px-4 py-3 text-white">{{loading?'Salvando...':'Salvar'}}</button></div>
